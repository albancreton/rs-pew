use std::vec;

use ratatui::buffer::Buffer;
use ratatui::widgets::Widget;
use ratatui::{layout::Rect, style::Color};

use crate::noise::Perlin;

pub struct CaveConfig {
    pub opening_max: f64,
    pub opening_min: f64,
    pub frequency: f64,
    pub smooth: f64,
    pub color: Color,
}

#[derive(Clone, Copy)]
pub struct CaveModel {
    pub seed: f64,
    pub offset_x: i64,
    pub speed: i64,

    opening_ratio: f64,
    opening_min: f64,
    frequency: f64,
    smooth: f64,

    color: Color,
}

impl CaveModel {
    pub fn new(seed: f64, config: CaveConfig) -> Self {
        Self {
            seed: seed,

            offset_x: 0,
            speed: 0,

            opening_ratio: config.opening_max,
            opening_min: config.opening_min,
            frequency: config.frequency,
            smooth: config.smooth,

            color: config.color,
        }
    }
    pub fn scroll(&mut self) {
        self.offset_x += self.speed;
    }
    pub fn set_speed(&mut self, speed: i64) {
        self.speed = speed;
    }
    pub fn inc_speed(&mut self, by: i64) {
        self.speed = self.speed + by;
    }

    pub fn calculate(&self, area: Rect) -> Vec<(u16, u16)> {
        let noise_base_x = area.x as f64 + self.offset_x as f64;

        let mut perlin_top = Perlin::new(self.seed, true);
        perlin_top.set_interval(0.2, 0.8);

        let mut perlin_bot = Perlin::new(self.seed + 25.0, true);
        perlin_bot.set_interval(self.opening_min, self.opening_ratio);

        let frequency = self.frequency;
        let smooth = self.smooth;

        let mut buf: Vec<(u16, u16)> = vec![];

        for (_, x) in (area.left()..area.right()).enumerate() {
            let noise_x = x as f64 + noise_base_x;

            let noise_center = perlin_top.noise2d(noise_x / frequency, 0.0);
            let noise_opening = perlin_bot.noise2d(noise_x / smooth, 2.0);

            let center = area.height as f64 * noise_center;
            let opening = area.height as f64 * noise_opening;

            let top = (center - (opening / 2.0)) as u16;
            let bot = (center + (opening / 2.0)) as u16;

            /*
            -
            | Solid
            | Solid
            - center - opening / 2
            |
            - center
            |
            - center + opening / 2
            | Solid
            | Solid
            -
            */
            for (_, y) in (area.top()..area.bottom()).enumerate() {
                if y < top || y > bot {
                    buf.push((x as u16, y as u16));
                }
            }
        }

        buf
    }
}

pub struct CaveWidget<'a> {
    model: &'a CaveModel,
}

impl CaveWidget<'_> {
    pub fn new(model: &CaveModel) -> CaveWidget {
        CaveWidget { model }
    }
}

impl Widget for CaveWidget<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let points = self.model.calculate(area);
        for (x, y) in points.iter() {
            buf.get_mut(*x, *y).set_char('█').set_fg(self.model.color);
        }
    }
}
